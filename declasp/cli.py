...
def run_query_checking(args):
    print(
        "Query checking is currently not implemented in the CLI, sorry!\nRefer to Python examples!"
    )
    sys.exit(0)
...
def main():
    ...
    parser_conformance.add_argument(
        "model", type=str, help="Path to a Declare model in JSON format."
    )
    parser_conformance.add_argument(
        "log", type=str, help="Path to an event log in XES format."
    )
    ...
    parser_satisfiability.add_argument(
        "model", type=str, help="Path to a Declare model in JSON format."
    )
    parser_satisfiability.add_argument(
        "horizon", type=int, help="Maximum length for the searched model."
    )
